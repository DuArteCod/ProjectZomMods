...
local function addVerdinhaToDistributions()
    ...
    for distName, dist in pairs(Distributions) do
        -- estrutura comum: dist.items eh uma tabela intercalada {item, weight, item, weight, ...}
        if dist.items then
            local i = 1
            while i <= #dist.items do
                local itemType = dist.items[i]
                if itemType == "Cigarettes" then
                    -- inserir logo apos a entrada atual (mantendo pesos baixos para packs/boxes)
    ...
    print("Verdinha: distribuicoes modificadas")
end
